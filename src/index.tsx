...
app.post('/api/analyze', async (c) => {
  try {
    ...
    // v2: ML API統合対応（古いキャッシュを無効化）
    const cacheKey = `analysis:v2:${symbol}:${trainModel}:${enableBackfit}`
    
    // キャッシュチェック（デバッグ中は無効化）
    const ENABLE_CACHE = false  // デバッグ後はtrueに戻す
    const cached = ENABLE_CACHE ? cache.get(cacheKey) : null
    if (cached) {
      console.log(`[CACHE] Returning cached analysis for ${symbol}`)
      return c.json(cached)
    } else if (!ENABLE_CACHE) {
      console.log(`[CACHE] Cache disabled for debugging`)
    }
    ...
    // ML予測を生成（並行表示用）
    console.log(`[ML] Calling generateMLPrediction for ${symbol}...`)
    console.log(`[ML] Parameters: trainModel=${trainModel}, enableBackfit=${enableBackfit}, ML_API_URL=${env.ML_API_URL}`)
    ...
    console.log(`[ML] generateMLPrediction completed`)
    console.log(`[ML] Result summary:`, {
      has_prediction: !!mlResult.prediction,
      has_training: !!mlResult.training,
      has_future_predictions: !!mlResult.training?.future_predictions
    })
    if (mlResult.training) {
    ...
    try {
      gpt5FinalJudgment = await generateGPT5FinalJudgment(
        symbol,
        currentPrice,
        prediction,
        technical,
      ...
      console.error('[WARN] GPT-5 final judgment failed, continuing without it:', gpt5Error)
      gpt5FinalJudgment = null
    }
    ...
    const result = {
      ...
      // デバッグ情報
      debug: {
        ml_api_url_configured: !!env.ML_API_URL,
        ml_api_url: env.ML_API_URL ? `${env.ML_API_URL.substring(0, 30)}...` : 'NOT SET',
        train_model_requested: trainModel,
        ml_prediction_exists: !!mlResult.prediction,
        ml_training_exists: !!mlResult.training
      }
    }
  ...
  <script>
    ...
    async function analyzeStock() {
      ...
      try {
                  ...
                  <div class="space-y-3">
                    <div>
                      ...
                      <p class="text-3xl font-bold text-green-600">
                        $\${(data.prediction.ml_prediction?.predicted_price || 0).toFixed(2)}
                      </p>
                      <p class="text-sm \${data.prediction.ml_prediction.change_percent > 0 ? 'text-green-600' : 'text-red-600'}">
                        \${data.prediction.ml_prediction.change_percent > 0 ? '+' : ''}\${(data.prediction.ml_prediction?.change_percent || 0).toFixed(2)}%
                      </p>
                    </div>
                  ...
                  <div class="mt-4 bg-white p-4 rounded-lg shadow">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div class="bg-blue-50 p-3 rounded-lg text-center">
                        ...
                        <p class="text-xl font-bold text-blue-600">\${(data.prediction.ml_prediction.model_metrics?.mae || 0).toFixed(2)}</p>
                        <p class="text-xs text-gray-500 mt-1">低いほど高精度</p>
                      </div>
                      <div class="bg-green-50 p-3 rounded-lg text-center">
                        <p class="text-xs text-gray-600 mb-1">RMSE (平均二乗誤差)</p>
                        <p class="text-xl font-bold text-green-600">\${(data.prediction.ml_prediction.model_metrics?.rmse || 0).toFixed(2)}</p>
                        <p class="text-xs text-gray-500 mt-1">低いほど高精度</p>
                      </div>
                      <div class="bg-purple-50 p-3 rounded-lg text-center">
                        <p class="text-xs text-gray-600 mb-1">R² スコア</p>
                        <p class="text-xl font-bold text-purple-600">\${(data.prediction.ml_prediction.model_metrics?.r2_score || 0).toFixed(3)}</p>
              ...
              <div class="grid grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white p-4 rounded-lg shadow">
                  ...
                  <p class="text-2xl font-bold text-purple-700">
                    \${(data.prediction.ml_training?.training_duration || 0).toFixed(1)}秒
                  </p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow">
                  <p class="text-xs text-gray-600 mb-1">学習サンプル数</p>
                  <p class="text-2xl font-bold text-purple-700">
                    \${data.prediction.ml_training.accuracy_metrics?.training_samples}
                  </p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow">
                  <p class="text-xs text-gray-600 mb-1">テストサンプル数</p>
                  <p class="text-2xl font-bold text-purple-700">
                    \${data.prediction.ml_training.accuracy_metrics?.validation_samples}
                  </p>
                </div>
              </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-2">
                    <div class="flex justify-between">
                      ...
                      <span class="font-bold">\${(data.prediction.ml_training.accuracy_metrics?.training_samples || 0) + (data.prediction.ml_training.accuracy_metrics?.validation_samples || 0)}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600">学習セット:</span>
                      <span class="font-bold text-blue-600">\${data.prediction.ml_training.accuracy_metrics?.training_samples}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600">テストセット:</span>
                      <span class="font-bold text-green-600">\${data.prediction.ml_training.accuracy_metrics?.validation_samples}</span>
                    </div>
                  </div>
                  <div class="space-y-2">
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                      ...
                      <span class="font-bold text-xs">\${new Date().toLocaleString('ja-JP', {timeZone: 'Asia/Tokyo'})}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-white p-6 rounded-lg shadow-lg mb-6">
                ...
                <div class="grid grid-cols-3 gap-6 mb-4">
                  ...
                  <div class="text-center bg-blue-50 p-4 rounded">
                    ...
                    <p class="text-2xl font-bold text-blue-600">\${(data.prediction.ml_training.accuracy_metrics?.rmse || 0).toFixed(2)}</p>
                  </div>

                  <!-- MAE -->
                  <div class="text-center bg-green-50 p-4 rounded">
                    <p class="text-sm text-gray-600 mb-2">MAE（平均絶対誤差）</p>
                    <p class="text-2xl font-bold text-green-600">\${(data.prediction.ml_training.accuracy_metrics?.mae || 0).toFixed(2)}</p>
                  </div>

                  <!-- R² Score -->
                  <div class="text-center bg-purple-50 p-4 rounded">
                    <p class="text-sm text-gray-600 mb-2">R²スコア（決定係数）</p>
                    <p class="text-2xl font-bold text-purple-600">\${(data.prediction.ml_training.accuracy_metrics?.r2_score || 0).toFixed(4)}</p>
                  </div>
                </div>
        ...
        // 特徴量重要度チャート（学習結果用）
        if (data.prediction.ml_training && data.prediction.ml_training.feature_importance) {
          // 特徴量重要度チャート
          const featureImportanceTrainingCtx = document.getElementById('featureImportanceTrainingChart')?.getContext('2d')
          if (!featureImportanceTrainingCtx) {
            console.warn('[WARN] featureImportanceTrainingChart element not found, skipping...')
          } else {
          const trainingData = data.prediction.ml_training
          const topFeatures = trainingData.feature_importance.slice(0, 10)
