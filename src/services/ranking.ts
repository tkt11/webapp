...
async function analyzeLightweight(
  ...
  try {
    ...
    if (!quote) {
      console.error(`[ERROR] ${symbol}: No quote data, skipping`)
      return null
    }
...
async function getQuoteData(symbol: string, apiKey: string): Promise<{ price: number } | null> {
  try {
    ...
    return await getCachedData(cacheKey, CACHE_TTL.REALTIME_QUOTE, async () => {
      console.log(`[SEARCH] Fetching quote for ${symbol} from Finnhub...`)
      const url = `https://finnhub.io/api/v1/quote?symbol=${symbol}&token=${apiKey}`
      const response = await fetch(url)
      
      if (!response.ok) {
        console.error(`[ERROR] Finnhub API error for ${symbol}: ${response.status} ${response.statusText}`)
        return null
      }
      
      const data = await response.json()
      console.log(`[QUOTE] ${symbol} quote response:`, JSON.stringify(data).substring(0, 200))
      
      if (data.c && data.c > 0) {
        console.log(`[OK] ${symbol} price: $${data.c}`)
        return { price: data.c }
      }
      
      console.warn(`[WARN] ${symbol}: Invalid quote data (c=${data.c})`)
      return null
    })
  } catch (error) {
...
export async function getRecommendedRanking(
  ...
  // API キーの検証
  console.log(`[API-KEY] API Keys check:`)
  ...
  if (cached) {
    console.log('[OK] Returning cached recommended ranking')
    return {
      ...cached,
      metadata: {
        ...cached.metadata,
  ...
  console.log('[START] Starting recommended ranking generation...')
