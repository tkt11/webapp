...
export function performFundamentalAnalysis(metrics: FinancialMetrics): FundamentalAnalysis {
  ...
  if (metrics.pe_ratio !== null) {
    if (metrics.pe_ratio < 15) {
      score += 20
      signals.push(`✅ PER割安 (${metrics.pe_ratio.toFixed(2)})`)
    } else if (metrics.pe_ratio > 30) {
      score -= 15
      signals.push(`❌ PER割高 (${metrics.pe_ratio.toFixed(2)})`)
    } else {
      signals.push(`⚪ PER適正 (${metrics.pe_ratio.toFixed(2)})`)
    }
  } else {
    signals.push('⚠️ PERデータなし')
  }
  ...
  if (metrics.pb_ratio !== null) {
    if (metrics.pb_ratio < 1) {
      score += 15
      signals.push(`✅ PBR超割安 (${metrics.pb_ratio.toFixed(2)})`)
    } else if (metrics.pb_ratio > 3) {
      score -= 10
      signals.push(`❌ PBR割高 (${metrics.pb_ratio.toFixed(2)})`)
    } else {
      signals.push(`⚪ PBR適正 (${metrics.pb_ratio.toFixed(2)})`)
    }
  } else {
    signals.push('⚠️ PBRデータなし')
  }
  ...
  if (metrics.roe !== null) {
    if (metrics.roe > 15) {
      score += 15
      signals.push(`✅ ROE優良 (${metrics.roe.toFixed(2)}%)`)
    } else if (metrics.roe < 5) {
      score -= 10
      signals.push(`❌ ROE低迷 (${metrics.roe.toFixed(2)}%)`)
    } else {
      signals.push(`⚪ ROE標準 (${metrics.roe.toFixed(2)}%)`)
    }
  } else {
    signals.push('⚠️ ROEデータなし')
  }
  ...
  if (metrics.dividend_yield !== null) {
    if (metrics.dividend_yield > 3) {
      score += 10
      signals.push(`✅ 高配当 (${metrics.dividend_yield.toFixed(2)}%)`)
  ...
  if (metrics.revenue_growth !== null) {
    if (metrics.revenue_growth > 20) {
      score += 10
      signals.push(`✅ 高成長 (売上+${metrics.revenue_growth.toFixed(1)}%)`)
    } else if (metrics.revenue_growth < 0) {
      score -= 10
      signals.push(`❌ 売上減少 (${metrics.revenue_growth.toFixed(1)}%)`)
    }
  }
  ...
  if (metrics.profit_margin !== null) {
    if (metrics.profit_margin > 20) {
      score += 5
      signals.push(`✅ 高利益率 (${metrics.profit_margin.toFixed(1)}%)`)
    } else if (metrics.profit_margin < 5) {
      score -= 5
      signals.push(`❌ 低利益率 (${metrics.profit_margin.toFixed(1)}%)`)
    }
  }
  ...
  if (metrics.debt_to_equity !== null) {
    if (metrics.debt_to_equity < 0.5) {
      score += 5
      signals.push(`✅ 健全な財務 (D/E: ${metrics.debt_to_equity.toFixed(2)})`)
    } else if (metrics.debt_to_equity > 2) {
      score -= 5
      signals.push(`❌ 高負債 (D/E: ${metrics.debt_to_equity.toFixed(2)})`)
    }
  }
